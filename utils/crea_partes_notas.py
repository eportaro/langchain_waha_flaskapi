...
class Notes:
    ...
    def guardar_nota(transcripcion_path):
        ...
        transcripcion_path = str(transcripcion_path).replace("'", "").strip()
        print(f'transcripcion_path {transcripcion_path}')

        # Manejo robusto de la ruta
        if len(transcripcion_path.split()) > 3:  # Si parece una descripción
            base_dir = '_transcripciones_extraídos_descargados'
            if os.path.exists(base_dir):
                # Buscar el archivo más reciente
                files = [os.path.join(base_dir, f) for f in os.listdir(base_dir) if f.endswith('.md')]
                if files:
                    transcripcion_path = max(files, key=os.path.getmtime)
                    print(f"Usando archivo más reciente: {transcripcion_path}")
        
        # Si no tiene ruta completa, intentar encontrarlo
        if not os.path.dirname(transcripcion_path):
            possible_path = os.path.join('_transcripciones_extraídos_descargados', transcripcion_path)
            if os.path.exists(possible_path):
                transcripcion_path = possible_path
        
        # Verificar que el archivo existe
        if not os.path.exists(transcripcion_path):
            raise FileNotFoundError(f"No se encontró el archivo: {transcripcion_path}")
